...
[Light introduction to ANSI escape codes](https://notes.burke.libbey.me/ansi-escape-codes/)
...
CSI stands for Control Sequence Introducer. `ESC` followed by `[` is interpreted
as a control sequence beginning, followed by a sequence of parameter bytes and
then a final command character specifying the command.

Sequences use the layout below. Numerical parameters are expressed as
concatenated ASCII digit bytes. For example, the number `15` is expressed as the
ASCII character byte for `1`, followed by the ASCII character byte for `5`.
